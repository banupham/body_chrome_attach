...
from .paths import default_desktop_root, resolve_desktop_paths
...
def load_runtime_config(path: Path | str = DEFAULT_CONFIG_PATH) -> RuntimeConfig:
    ...
    return RuntimeConfig(host, port, protocol, contract, startup_timeout, readiness_poll)


def local_app_data_dir() -> Path:
    return default_desktop_root()


def ensure_runtime_dirs() -> dict[str, Path]:
    return resolve_desktop_paths().ensure().as_dict()
