...
class DaemonBridge{
  ...
  async forwardUserMotor(tabId,payload){if(!this.recordingEnabled||payload?.source!=='USER')return false;const event=payload.event||{};let context={...(payload.context||{})};const pointerBoundary=payload.kind==='pointer'&&['mousePressed','mouseReleased'].includes(String(event.type));const needsTargetContext=payload.kind==='keyboard'||pointerBoundary;if(needsTargetContext){try{const response=await this.chrome.tabs.sendMessage(Number(tabId),{action:'body.targetContextAt',x:event.x,y:event.y});if(response?.ok&&response.result)context={...context,...response.result};}catch{}}const tag=context.tag||null,inputType=context.inputType||null,sensitive=context.sensitive===true||inputType==='password';const target={tag,role:context.role||null,inputType,editable:context.editable===true||(!sensitive&&['input','textarea'].includes(tag)),sensitive,rect:context.rect||null};let row=null;if(payload.kind==='pointer'){const eventType=pointerEventType(event.type);if(eventType)row={eventType,ts:Number(event.at||Date.now()),source:'human',sourceConfidence:1,agentCommandId:null,agentStepIndex:null,x:Number(event.x),y:Number(event.y),button:event.button,buttons:Number(event.buttons||0),deltaX:Number(event.deltaX||0),deltaY:Number(event.deltaY||0),target,isTrusted:true};}else if(payload.kind==='keyboard'){const rawKey=String(event.key||''),printable=rawKey.length===1;row={eventType:event.type,ts:Number(event.at||Date.now()),source:'human',sourceConfidence:1,agentCommandId:null,agentStepIndex:null,key:sensitive||printable?null:rawKey,keyClass:sensitive?'redacted':this.keyClass(rawKey),code:sensitive||printable?null:event.code,repeat:event.repeat===true,target,isTrusted:true};}if(!row)return false;return this.send({type:'RECORDER_EVENT',tabId:Number(tabId),siteKey:siteKeyFromUrl(payload.url),event:row});}
  ...
  installTabListeners(){this.chrome.tabs.onActivated?.addListener(async info=>{const tabId=Number(info.tabId);let tab;try{tab=await this.chrome.tabs.get(tabId);}catch{return;}const marker=this.expectedTabSwitch.get(tabId),switchAgent=marker&&marker.expiresAt>Date.now();if(marker)this.expectedTabSwitch.delete(tabId);const browserMarker=this.expectedBrowserUi&&this.expectedBrowserUi.expiresAt>Date.now()?this.expectedBrowserUi:null;const agent=Boolean(switchAgent||browserMarker);this.send({type:'TAB_EVENT',event:{eventType:'tabActivated',tabId,windowId:tab.windowId,title:tab.title||'',siteKey:siteKeyFromUrl(tab.url),source:agent?'agent':'human',agentCommandId:switchAgent?marker.commandId:browserMarker?.commandId||null,browserAction:browserMarker?.action||null,ts:Date.now()}});});this.chrome.tabs.onUpdated?.addListener((tabId,changeInfo,tab)=>{if(changeInfo.url||changeInfo.title||changeInfo.status)this.navigationEpochByTab.set(Number(tabId),Number(this.navigationEpochByTab.get(Number(tabId))||0)+1);if(!changeInfo.url&&!changeInfo.title&&!changeInfo.status)return;this.send({type:'TAB_CONTEXT',tabId:Number(tabId),context:{siteKey:siteKeyFromUrl(tab.url),navigationToken:navigationToken(tab.url),navigationEpoch:Number(this.navigationEpochByTab.get(Number(tabId))||0),title:tab.title||'',windowId:tab.windowId,status:tab.status||changeInfo.status||null}});});this.chrome.tabs.onRemoved?.addListener(tabId=>{this.navigationEpochByTab.delete(Number(tabId));this.send({type:'TAB_REMOVED',tabId:Number(tabId)});});}
