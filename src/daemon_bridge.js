...
class DaemonBridge{
  ...
  async bodyObservationSnapshot(tabId){
    const id=Number(tabId);if(!Number.isInteger(id))throw new Error('invalid_tab_id');
    const tab=await this.chrome.tabs.get(id),observedAt=Date.now();
    const [page,semantic,pointer]=await Promise.all([this.pageObservation(id),this.semanticObservation(id),this.pointerStatus(id)]);
    let urlScheme='';try{urlScheme=new URL(tab.url).protocol;}catch{}
    return {observedAt,tab:{id,active:tab.active===true,windowId:Number.isInteger(Number(tab.windowId))?Number(tab.windowId):null,title:tab.title||'',siteKey:siteKeyFromUrl(tab.url),navigationToken:navigationToken(tab.url),navigationEpoch:Number(this.navigationEpochByTab.get(id)||0),status:tab.status||null,urlScheme},page,semantic,pointer};
  }
  ...
  async handle(msg){
    ...
    if(msg.type==='BODY_OBSERVE_SNAPSHOT')return this.bodyObservationSnapshot(Number(msg.tabId));
