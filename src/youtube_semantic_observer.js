...
function extractSurface(documentRef,surface,{windowRef=globalThis.window,maxItems=50}={}){
  ...
  const semanticCount=items.filter(x=>x.semanticTitle&&x.title).length;return {surface,itemCount:items.length,scrollRect:surface==='mix_queue'?rootRect:null,diagnostics:{rootSelector,candidateAnchors:anchors.length,extractedItems:items.length,semanticTitleItems:semanticCount,semanticCoverage:items.length?semanticCount/items.length:0,extractorVersion:7},items};
}
...
function youtubeSemanticObservation({documentRef=globalThis.document,windowRef=globalThis.window,locationRef=globalThis.location,maxItems=50}={}){
  const route=youtubeRoute(locationRef);if(!route.supported)return {available:false,platform:null,observerVersion:4,reason:'unsupported_site'};
  ...
  return {available:true,platform:'youtube',observerVersion:4,observedAt:Date.now(),privacy:{searchQueryCaptured:false,searchQueryFingerprintCaptured:true,accountIdentityCaptured:false,textContentCaptured:false,candidateSemanticTextCaptured:true,genericAffordanceLabelsCaptured:true,inputValuesCaptured:false},route,currentVideo:currentVideoDescriptor(documentRef,route),signedInState:signedInState(documentRef),controls,advertising,affordances,surfaces,viewport:viewportState(documentRef,windowRef)};
}
