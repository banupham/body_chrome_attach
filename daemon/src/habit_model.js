...
function tabStateKey(browserInstanceId,tabId){const browser=String(browserInstanceId||'').trim();const tab=String(tabId??'unknown');return browser?`${browser}::${tab}`:tab;}

class HabitModel {
  constructor(modelPath,{persistenceOptions={}}={}) {
    this.modelPath=modelPath;
    this.state={version:1,revision:0,updatedAt:null,transitions:{},habits:{},lastHumanByTab:{}};
  ...
  save({immediate=false}={}) {this.persistence.schedule();return immediate ? this.persistence.flushSync() : this.persistence.status();}
  flushSync(){ return this.persistence.flushSync(); }
  _inc(obj,key,by=1) {obj[key]=(obj[key]||0)+by;}

  _observeNamedHabits(prev,current) {
    if(!prev || !current) return;
    if(prev.action==='typeText') {
      if(current.action==='pressKey' && String(current.key)==='Tab') this._habit('after_typing_next_focus','keyboard_tab');
      else if(current.action==='click' && current.context?.target_editable===true) this._habit('after_typing_next_focus','mouse_click');
      if(current.action==='pressKey' && String(current.key)==='Enter') this._habit('after_typing_submit','keyboard_enter');
      else if(current.action==='click' && ['button','submit'].includes(targetRole(current))) this._habit('after_typing_submit','mouse_click');
      if(current.action==='pressKey' && String(current.key)==='Escape') this._habit('after_typing_dismiss','keyboard_escape');
    }
    ...
    const t=this.state.transitions[key];t.total++;this._inc(t.modalities,modalityOf(current));this._inc(t.actions,current.action);
  }

  _habit(key,strategyId) {
    if(!this.state.habits[key]) this.state.habits[key]={total:0,strategies:{}};
    const h=this.state.habits[key];h.total++;this._inc(h.strategies,strategyId);
  }
  ...
  scoreAlternatives({habitKey,alternatives=[],context={}}) {
    ...
    const prevAction=String(context.previousAction||'unknown'),prevModality=String(context.previousModality||'unknown'),role=String(context.targetRole||'unknown').toLowerCase();
    ...
    const scored=alternatives.map((alt,index)=>{
      const id=String(alt.id||`strategy_${index}`),modality=String(alt.modality||'other'),habitCount=Number(h.strategies[id]||0),modalityCount=Number(tr?.modalities?.[modality]||0),actionCount=(alt.actions||[]).reduce((sum,a)=>sum+Number(tr?.actions?.[a.type]||0),0);
      ...
      return {id,modality,score:habitProb*0.62 + modalityProb*0.23 + actionProb*0.15,habitCount,transitionModalityCount:modalityCount,transitionActionCount:actionCount,index,alternative:alt};
    });
    scored.sort((a,b)=>{if(b.score!==a.score)return b.score-a.score;if(b.habitCount!==a.habitCount)return b.habitCount-a.habitCount;return String(a.id).localeCompare(String(b.id),'en');});
