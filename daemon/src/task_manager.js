...
class TaskManager{
  constructor(baseDir,browserManager,{uuid=()=>crypto.randomUUID(),now=()=>Date.now(),env=process.env}={}){
    ...
    this.browserManager=browserManager;this.uuid=uuid;this.now=now;this.dir=path.join(resolvedBaseDir,'state');this.file=path.join(this.dir,'tasks.json');fs.mkdirSync(this.dir,{recursive:true});
  ...
  create(spec={}){
    ...
    const workspace=this._assertWorkspace(browserId,spec.primaryTabId,spec.tabIds,spec.workspaceMode),now=this._ts(),state='READY';
    const task={taskId,capability:String(spec.capability||spec.goal?.capability||''),goal:clone(spec.goal||null),taskType:spec.taskType||null,state,workspace,createdAt:now,updatedAt:now,result:null,error:null};
    return this._mutate(()=>{this._claim(task);this.state.tasks[taskId]=task;return clone(task);});
  }
  ...
  approve(taskId){return this._mutate(()=>{const task=this.get(taskId);if(task.state!=='WAITING_APPROVAL')throw new Error(`task_not_waiting_approval:${task.taskId}`);this._claim(task);task.state='READY';task.updatedAt=this._ts();return clone(task);});}
  ...
  executionContext(taskId,tabRef='primary',{autoStart=true}={}){
    ...
    return {taskId:task.taskId,browserInstanceId:browser.browserInstanceId,extensionInstanceId:browser.extensionInstanceId,tabId:Number(tabId)};
  }
...
module.exports={TaskManager,TASK_STATES,WORKSPACE_MODES};
