...
function privacySafe(observation){return observation?.privacy?.searchQueryCaptured!==true&&observation?.privacy?.accountIdentityCaptured!==true&&observation?.privacy?.textContentCaptured!==true;}
function isYoutubeSearchTrigger(event){const before=event?.semanticBefore;return event?.source==='human'&&event?.isTrusted===true&&event?.eventType==='keydown'&&event?.key==='Enter'&&privacySafe(before)&&before?.available===true&&before?.platform==='youtube'&&before?.controls?.searchInput?.active===true;}
...
function observedEffect(before,after){
  ...
  return {navigationObserved:pageTypeChanged||searchSurfaceAppeared||searchResultCountChanged,pageTypeChanged,fromPageType:beforeType,toPageType:afterType,searchResultsObserved:afterType==='search'&&afterCount!==null,searchResultCount:afterCount===null?0:afterCount,searchResultCountChanged};
}

class EvidenceAssembler{
  constructor(store,{now=()=>Date.now(),ttlMs=10000}={}){if(!store)throw new Error('evidence_assembler_store_required');this.store=store;this.now=now;this.ttlMs=Math.max(1000,Number(ttlMs)||10000);this.pending=new Map();this.completed=0;this.expired=0;}
  ...
  observeRecorder(identity,siteKey,tabId,event){this._prune();if(!isYoutubeSearchTrigger(event))return null;const before=sanitizeObservation(event.semanticBefore);if(before.available!==true)return null;const key=pendingKey(identity,tabId);this.pending.set(key,{createdAt:this.now(),identity:clone(identity),siteKey:String(siteKey||'__unknown__'),tabId:Number(tabId),triggerTs:Number(event.ts||this.now()),before});return {accepted:true,key,action:'youtube.search'};}
  observeAfter(identity,siteKey,tabId,observation){
    this._prune();const key=pendingKey(identity,tabId),pending=this.pending.get(key);if(!pending)return null;
    const after=sanitizeObservation(observation);if(after.available!==true||after.route?.pageType!=='search')return null;
    if(after.observedAt&&pending.before.observedAt&&after.observedAt<pending.before.observedAt)return null;
    this.pending.delete(key);const effect=observedEffect(pending.before,after);
    const record=this.store.append({identity:pending.identity,siteKey:pending.siteKey||siteKey,tabId:pending.tabId,source:'human',provenance:{kind:'human_demonstration',trustedInput:true,triggerEventTs:pending.triggerTs,semanticObserver:'youtube-v1'},beforeState:pending.before,action:{type:'youtube.search',trigger:'keyboard_enter',queryCaptured:false},afterState:after,observedEffect:effect});
    this.completed++;return record;
  }
