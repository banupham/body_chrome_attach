...
function createDaemonRuntime({baseDir=path.join(__dirname,'..'),printAsync=()=>{},identityOptions={},taskOptions={},environmentOptions={}}={}){
  ...
  function extensionOnline(item){const browser=browsers.registerExtension(item),reconcile=tasks.reconcileBrowser(browser.browserInstanceId);return {browser,reconcile};}
  ...
  function segmenter(extId,tabId,siteKey){const key=segKey(extId,tabId,siteKey);if(segmenters.has(key))return segmenters.get(key);const s=new HumanActionSegmenter(sample=>{const learned=learning.observeHumanSample(extId,siteKey,{...sample,tabId:Number(tabId)},{learn:learningEnabled});printAsync(`[HỌC] ext=${String(extId).slice(0,8)} tab=${tabId} site=${normalizeSiteKey(siteKey)} action=${learned.action}`);});segmenters.set(key,s);return s;}
  ...
  async function executeStrategy(task,{extensionId=null,tabId='active'}={}){const extId=selected(extensionId);return execution.run(extId,{capability:'HUMAN_MOTOR',operation:'strategy',habitKey:String(task?.habitKey||'unknown')},async()=>{const tab=await resolveTab(extId,tabId),tid=Number(tab.id),siteKey=normalizeSiteKey(tab.siteKey||tabSites.get(ctx(extId,tid))),executor=new StrategyExecutor(learning.habitFor(extId,siteKey),new MotorPlanner(learning.motorFor(extId,siteKey))),decision=executor.choose(task,history(extId,siteKey,tid,task.targetRole||task.role)),planned=executor.planSelected(decision,{pointerStart:pointers.get(ctx(extId,tid))||{x:400,y:300}}),commandId=id(`strategy-${planned.strategyId}`),result=await requestExtension(extId,'BODY_EXECUTE',{commandId,tabId:tid,deadlineMs:Number(task.deadlineMs||60000),plan:planned.plan},65000);return {commandId,identity:identityForExtension(extId),extensionId:extId,tabId:tid,siteKey,habitScope:decision.scopeSource||'site',habitKey:task.habitKey,selectedStrategy:planned.strategyId,selectedModality:planned.modality,score:planned.score,habitObservations:decision.totalHabitObservations,ranking:decision.ranking.map(x=>({id:x.id,modality:x.modality,score:x.score,habitCount:x.habitCount,transitionModalityCount:x.transitionModalityCount,transitionActionCount:x.transitionActionCount})),subPlans:planned.subPlans,execution:result};});}
