...
function createDaemonRuntime({baseDir=path.join(__dirname,'..'),printAsync=()=>{},identityOptions={},taskOptions={}}={}){
  ...
  function identityForExtension(extensionId=null){
    const item=registry.get(extensionId||null);
    if(item?.browserInstanceId){const chain=identity.identityChain(item.browserInstanceId);if(chain)return chain;}
    return {...identity.snapshot(),browserInstanceId:item?.browserInstanceId||null,extensionInstanceId:item?.extensionInstanceId||item?.extensionId||extensionId||null,runtimeExtensionId:item?.runtimeExtensionId||null,platformIdentities:{}};
  }
  function extensionOnline(item){const browser=browsers.registerExtension(item);const reconcile=tasks.reconcileBrowser(browser.browserInstanceId);return {browser,reconcile};}
  ...
  function requestExtension(extensionId,type,payload={},timeoutMs=30000){
    ...
    return new Promise((resolve,reject)=>{
      ...
      if(!send(ext.ws,{type,requestId,...payload})){clearTimeout(timer);pending.delete(requestId);reject(new Error(`extension_not_connected:${extensionId}:${type}`));}
    });
  }
  function rejectPendingForExtension(extensionId,reason='extension_disconnected'){
    const extId=String(extensionId);let rejected=0;
    for(const [requestId,p] of [...pending.entries()]){if(String(p.extensionId)!==extId)continue;clearTimeout(p.timer);pending.delete(requestId);rejected++;p.reject(new Error(`${reason}:${extId}:${p.type||'request'}`));}
    return rejected;
  }
  function resolveResponse(extId,msg){
    if(msg.type!=='RESPONSE'||!msg.requestId)return false;const p=pending.get(msg.requestId);if(!p)return true;
    clearTimeout(p.timer);pending.delete(msg.requestId);if(p.extensionId!==extId)p.reject(new Error('extension_response_scope_mismatch'));else if(msg.ok)p.resolve(msg.result);else p.reject(new Error(msg.error?.message||'extension_error'));return true;
  }
  ...
  async function resolveTab(extId,ref='active'){
    ...
    if(ref==='active'||ref==null){
      ...
      const tab=await requestExtension(extId,'ACTIVE_TAB');registry.activateTab(extId,tab.id,tab);browsers.updateTabByExtension(extId,tab.id,{...tab,active:true});tabSites.set(ctx(extId,tab.id),normalizeSiteKey(tab.siteKey));return tab;
    }
    const tabId=Number(ref);if(!Number.isInteger(tabId))throw new Error('invalid_tab_id');let tab=ext.tabs.get(tabId);if(!tab){await refreshTabs(extId);tab=registry.require(extId).tabs.get(tabId);}if(!tab)throw new Error(`tab_not_found:${extId}:${tabId}`);return tab;
  }
  ...
  function segmenter(extId,tabId,siteKey){
    ...
    const s=new HumanActionSegmenter(sample=>{const learned=learning.observeHumanSample(extId,siteKey,{...sample,tabId:Number(tabId)},{learn:learningEnabled});printAsync(`[HỌC] ext=${String(extId).slice(0,8)} tab=${tabId} site=${normalizeSiteKey(siteKey)} action=${learned.action}`);});segmenters.set(key,s);return s;
  }
  ...
  function recorderEvent(extId,msg){
    const tabId=Number(msg.tabId),event=msg.event;if(!event)return;const siteKey=normalizeSiteKey(msg.siteKey||tabSites.get(ctx(extId,tabId))||'__unknown__');
  ...
  function extensionOffline(extId){const rejected=rejectPendingForExtension(extId,'extension_disconnected');const segments=disposeSegmentersForExtension(extId,{flush:true});browsers.extensionOffline(extId);for(const key of [...pointers.keys()])if(key.startsWith(`${String(extId)}/`))pointers.delete(key);for(const key of [...tabSites.keys()])if(key.startsWith(`${String(extId)}/`))tabSites.delete(key);return {rejectedPending:rejected,...segments};}

  async function executeIntent(intent,{extensionId=null,tabId='active'}={}){
    const extId=selected(extensionId);return execution.run(extId,{capability:'HUMAN_MOTOR',operation:'intent',action:String(intent?.type||'unknown')},async()=>{const tab=await resolveTab(extId,tabId),tid=Number(tab.id),siteKey=normalizeSiteKey(tab.siteKey||tabSites.get(ctx(extId,tid)));const planner=new MotorPlanner(learning.motorFor(extId,siteKey)),planned=planner.plan(intent,{pointerStart:pointers.get(ctx(extId,tid))||{x:400,y:300}}),commandId=id(intent.type||'action');const result=await requestExtension(extId,'BODY_EXECUTE',{commandId,tabId:tid,deadlineMs:Number(intent.deadlineMs||60000),plan:planned.plan},65000);return {commandId,identity:identityForExtension(extId),extensionId:extId,tabId:tid,siteKey,behaviorSource:planned.source,learnedGroup:planned.learnedGroup,learnedTemplateCount:planned.learnedTemplateCount,execution:result};});
  }
  ...
  async function executeStrategy(task,{extensionId=null,tabId='active'}={}){
    const extId=selected(extensionId);return execution.run(extId,{capability:'HUMAN_MOTOR',operation:'strategy',habitKey:String(task?.habitKey||'unknown')},async()=>{const tab=await resolveTab(extId,tabId),tid=Number(tab.id),siteKey=normalizeSiteKey(tab.siteKey||tabSites.get(ctx(extId,tid)));const executor=new StrategyExecutor(learning.habitFor(extId,siteKey),new MotorPlanner(learning.motorFor(extId,siteKey))),decision=executor.choose(task,history(extId,siteKey,tid,task.targetRole||task.role));const planned=executor.planSelected(decision,{pointerStart:pointers.get(ctx(extId,tid))||{x:400,y:300}}),commandId=id(`strategy-${planned.strategyId}`);const result=await requestExtension(extId,'BODY_EXECUTE',{commandId,tabId:tid,deadlineMs:Number(task.deadlineMs||60000),plan:planned.plan},65000);return {commandId,identity:identityForExtension(extId),extensionId:extId,tabId:tid,siteKey,habitScope:decision.scopeSource||'site',habitKey:task.habitKey,selectedStrategy:planned.strategyId,selectedModality:planned.modality,score:planned.score,habitObservations:decision.totalHabitObservations,ranking:decision.ranking.map(x=>({id:x.id,modality:x.modality,score:x.score,habitCount:x.habitCount,transitionModalityCount:x.transitionModalityCount,transitionActionCount:x.transitionActionCount})),subPlans:planned.subPlans,execution:result};});
  }
  async function switchTab(extensionId,tabId){const extId=selected(extensionId);return execution.run(extId,{capability:'BROWSER_UI',operation:'tab_switch',tabId:Number(tabId)},async()=>{const tab=await resolveTab(extId,tabId),commandId=id('switch-tab');const result=await requestExtension(extId,'TAB_SWITCH',{commandId,tabId:Number(tab.id)});registry.activateTab(extId,Number(tab.id),{...tab,active:true,siteKey:normalizeSiteKey(result.siteKey||tab.siteKey)});browsers.updateTabByExtension(extId,Number(tab.id),{...tab,active:true,siteKey:normalizeSiteKey(result.siteKey||tab.siteKey)});return {identity:identityForExtension(extId),extensionId:extId,commandId,source:'agent',...result};});}
  const runWindowsInput=createWindowsInputRunner({baseDir});
  async function browserSnapshot(extId){const tabs=await refreshTabs(extId);let active=tabs.find(t=>t.active)||null;try{active=await requestExtension(extId,'ACTIVE_TAB');}catch{}return {tabs,active};}
  ...
  function executeBrowserCommand(action,{extensionId=null,tabId='active',value=null}={}){const extId=selected(extensionId);return execution.run(extId,{capability:'BROWSER_UI',operation:'browser_command',action:String(action||'unknown')},async()=>{const result=await browserUi.execute(action,{extensionId:extId,tabId,value});return {identity:identityForExtension(extId),...result};});}

  async function withTask(taskId,tabRef,work){
    const taskContext=tasks.executionContext(taskId,tabRef);const browserId=taskContext.browserInstanceId;browsers.setState(browserId,'BUSY',`task:${taskId}`);
    try{return await work(taskContext);}finally{const row=browsers.require(browserId);if(row.online&&row.state==='BUSY')browsers.setState(browserId,'ACTIVE','task_step_finished');}
  }
  ...
  function flushSync(){let emitted=0;for(const s of segmenters.values())emitted+=Number(s.flush()?.emitted||0);const learningResult=learning.flushSync(),tabHabitResult=tabHabit.flushSync(),identityResult=identity.flushSync(),taskResult=tasks.flushSync();return {segmenterEmitted:emitted,learning:learningResult,tabHabitOk:tabHabitResult.ok,identityOk:identityResult.ok,taskStateOk:taskResult.ok};}

  return {
    identity,registry,browsers,tasks,learning,tabHabit,execution,pending,pointers,tabSites,id,ctx,pnum,send,registerExtensionIdentity,identityForExtension,extensionOnline,
    requestExtension,rejectPendingForExtension,resolveResponse,selected,refreshTabs,resolveTab,recorderEvent,tabEvent,tabContext,tabRemoved,extensionOffline,disposeSegmentersForTab,disposeSegmentersForExtension,
    executeIntent,executeStrategy,switchTab,executeBrowserCommand,executeTaskIntent,executeTaskStrategy,executeTaskBrowserCommand,executeTaskTabSwitch,flushSync,
    get recordingEnabled(){return recordingEnabled;},set recordingEnabled(value){recordingEnabled=value===true;},get learningEnabled(){return learningEnabled;},set learningEnabled(value){learningEnabled=value===true;}
  };
}
