...
function shouldSuppressHumanEcho(execution,extensionId,event){
  if(event?.source!=='human')return false;
  const lane=execution?.status?.(extensionId);
...
function createDaemonRuntime({baseDir=path.join(__dirname,'..'),printAsync=()=>{},identityOptions={},taskOptions={}}={}){
  ...
  async function pointerContext(extId,tabId){
    const chain=identityForExtension(extId),tid=Number(tabId);let state=pointerState.get(chain,tid),resynced=false;
    if(!state){
      try{
        const remote=await requestExtension(extId,'POINTER_STATUS',{tabId:tid},5000);
        if(remote?.known===true&&finitePointerValue(remote.x)&&finitePointerValue(remote.y)){
          state=pointerState.update(chain,tid,{x:Number(remote.x),y:Number(remote.y)},{source:remote.source||'observed',eventType:'resync',at:remote.updatedAt});resynced=true;
        }
      }catch{}
    }
    return {state,pointerStart:state?{x:state.x,y:state.y}:null,resynced};
  }
  async function pointerStatus(extensionId=null,tabId='active'){const extId=selected(extensionId),tab=await resolveTab(extId,tabId),pointer=await pointerContext(extId,Number(tab.id));return {...(pointer.state||pointerState.snapshot(identityForExtension(extId),Number(tab.id))),resynced:pointer.resynced};}
  ...
  function tabEvent(extId,event,{allowHumanLearning=true}={}){const tabId=Number(event.tabId),siteKey=normalizeSiteKey(event.siteKey);tabSites.set(ctx(extId,tabId),siteKey);registry.activateTab(extId,tabId,{siteKey,title:event.title||'',windowId:event.windowId});browsers.updateTabByExtension(extId,tabId,{siteKey,title:event.title||'',windowId:event.windowId,active:true});const human=event.source==='human';if(!human||allowHumanLearning===true)tabHabit.observe(extId,{...event,siteKey});if(human&&allowHumanLearning===true)learning.observeHumanSample(extId,siteKey,{source:'human',action:'switchTab',tabId,context:{target_role:'browser_tab',target_site:siteKey,windowId:event.windowId}},{learn:learningEnabled});}
  ...
  async function executeIntent(intent,{extensionId=null,tabId='active'}={}){
    ...
    return execution.run(extId,{capability:'HUMAN_MOTOR',operation:'intent',action:String(intent?.type||'unknown')},async()=>{
      const tab=await resolveTab(extId,tabId),tid=Number(tab.id),siteKey=normalizeSiteKey(tab.siteKey||tabSites.get(ctx(extId,tid))),chain=identityForExtension(extId),pointer=await pointerContext(extId,tid),planner=new MotorPlanner(learning.motorFor(extId,siteKey)),planned=planner.plan(intent,pointer.pointerStart?{pointerStart:pointer.pointerStart}:{}),commandId=id(intent.type||'action'),result=await requestExtension(extId,'BODY_EXECUTE',{commandId,tabId:tid,deadlineMs:Number(intent.deadlineMs||60000),plan:planned.plan},65000),pointerAfter=pointerState.applyPlan(chain,tid,planned.plan,{source:'agent'});
      return {commandId,identity:chain,extensionId:extId,tabId:tid,siteKey,behaviorSource:planned.source,learnedGroup:planned.learnedGroup,learnedTemplateCount:planned.learnedTemplateCount,learnedSelection:planned.learnedSelection,learnedTemplateIndex:planned.learnedTemplateIndex,pointerResynced:pointer.resynced,pointerBefore:pointer.state?{...pointer.state}:pointerState.snapshot(chain,tid),pointerAfter,execution:result};
    });
  }
