...
function nativeWindow(value={}){return {handle:finite(value.handle),processId:finite(value.processId),processName:text(value.processName,160)||null,name:text(value.name,300)||null,className:text(value.className,160)||null,controlType:text(value.controlType,80)||null,rect:rect(value.rect),zOrder:finite(value.zOrder),windowIntersection:rect(value.windowIntersection),contentIntersection:rect(value.contentIntersection)};}
function nativeFacts(value={}){return {targetWindowHandle:finite(value.targetWindowHandle),targetWindowForeground:value.targetWindowForeground===true,contentRect:rect(value.contentRect),foregroundWindow:value.foregroundWindow?nativeWindow(value.foregroundWindow):null,focusedElement:value.focusedElement?control(value.focusedElement):null,topLevelOccluders:Array.isArray(value.topLevelOccluders)?value.topLevelOccluders.slice(0,20).map(nativeWindow):[]};}
function normalizeSnapshot(raw,request={}){
  ...
  return {available:raw?.available!==false,observed:raw?.observed===true,reason:text(raw?.reason,120)||null,confidence:text(raw?.confidence,40)||'unknown',source:'windows_uia_read_only',observedAt:finite(raw?.observedAt)||Date.now(),scope:{browserInstanceId:text(request.browserInstanceId,180)||null,tabId:finite(request.tabId),windowId:finite(request.windowId),expectedTitle:text(request.title,240)||null},window:raw?.window?{processId:finite(raw.window.processId),name:text(raw.window.name,300)||null,className:text(raw.window.className,120)||null,rect:rect(raw.window.rect)}:null,native:nativeFacts(raw?.native||{}),focusedControl,addressBar,tabs,controls,signature:text(raw?.signature,96)||null};
}
