...
const BROWSER_STATES=new Set(['REGISTERED','OFFLINE','ONLINE','BUSY','HUMAN_CONTROL','ERROR']);
...
class BrowserManager{
  ...
  registerExtension(item){
    const browserId=String(item?.browserInstanceId||'').trim(),extensionId=String(item?.extensionInstanceId||item?.extensionId||'').trim();
    ...
    row.tabs=new Map();
    for(const tab of item.tabs?.values?.()||[])if(Number.isInteger(Number(tab.id)))row.tabs.set(Number(tab.id),{...tab,id:Number(tab.id)});
  ...
  browserForExtension(extensionId){const browserId=this.extensionToBrowser.get(String(extensionId||''));return browserId?this.browsers.get(browserId)||null:null;}
  updateTabsByExtension(extensionId,tabs){const row=this.browserForExtension(extensionId);if(!row)return null;row.tabs.clear();for(const tab of tabs||[])if(Number.isInteger(Number(tab.id)))row.tabs.set(Number(tab.id),{...tab,id:Number(tab.id)});row.activeTabId=[...row.tabs.values()].find(t=>t.active)?.id??row.activeTabId;row.lastSeenAt=Date.now();return this.public(row);}
  updateTabByExtension(extensionId,tabId,patch={}){const row=this.browserForExtension(extensionId);if(!row)return null;const id=Number(tabId);if(!Number.isInteger(id))return null;const current=row.tabs.get(id)||{id};row.tabs.set(id,{...current,...patch,id});if(patch.active===true){for(const [key,tab] of row.tabs.entries())row.tabs.set(key,{...tab,active:key===id});row.activeTabId=id;}row.lastSeenAt=Date.now();return this.public(row);}
  removeTabByExtension(extensionId,tabId){const row=this.browserForExtension(extensionId);if(!row)return null;const id=Number(tabId);row.tabs.delete(id);if(row.activeTabId===id)row.activeTabId=null;row.lastSeenAt=Date.now();return this.public(row);}
  ...
  extensionOffline(extensionId){
    ...
    row.online=false;
    row.state='OFFLINE';
    row.stateReason='extension_offline';
    row.lastSeenAt=Date.now();
    return this.public(row);
  }

  setState(browserInstanceId,state,reason=null){
    const row=this.require(browserInstanceId),next=String(state||'').toUpperCase();
  ...
  require(browserInstanceId){
    ...
    if(!row)throw new Error('browser_not_found:'+id);
    return row;
  }
  ...
  assertTab(browserInstanceId,tabId,{requireOnline=true}={}){
    ...
    if(row.state==='ERROR')throw new Error('browser_unavailable:'+row.browserInstanceId+':'+row.state);
    const id=Number(tabId);
    if(!Number.isInteger(id))throw new Error('task_tab_id_invalid');
    const tab=row.tabs.get(id);
    if(!tab)throw new Error('tab_not_in_browser:'+row.browserInstanceId+':'+id);
    return tab;
  }
  ...
  public(row){
    return {
      ...
      tabCount:row.tabs.size,
      tabs:[...row.tabs.values()].map(t=>({...t}))
    };
  }
