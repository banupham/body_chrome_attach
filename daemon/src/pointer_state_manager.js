...
function tabIdOf(value){const id=Number(value);if(!Number.isInteger(id))throw new Error('pointer_tab_id_required');return id;}
function pointOf(value){const x=Number(value?.x),y=Number(value?.y);if(!Number.isFinite(x)||!Number.isFinite(y))throw new Error('pointer_coordinates_required');return {x,y};}
...
class PointerStateManager{
  ...
  update(ref,tabId,point,{source='unknown',eventType=null,at=null}={}){
    const browserInstanceId=browserIdOf(ref),id=tabIdOf(tabId),p=pointOf(point),updatedAt=Number.isFinite(Number(at))?Number(at):this.now();
    this.sequence+=1;
  ...
  observeRecorder(ref,tabId,event){
    ...
    if(!Number.isFinite(Number(event.x))||!Number.isFinite(Number(event.y)))return null;
  ...
  applyPlan(ref,tabId,plan,{source='agent',at=null}={}){
    ...
    for(const step of plan?.steps||[]){
      ...
      if(!Number.isFinite(Number(step.params?.x))||!Number.isFinite(Number(step.params?.y)))continue;
