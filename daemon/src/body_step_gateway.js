...
class BodyStepGateway{
  ...
  reservedUnknownResult(command){return {contractVersion:BODY_CONTRACT_VERSION,type:'BODY_STEP_RESULT',stepId:command.stepId,taskId:command.taskId,action:{kind:command.step.kind,name:actionName(command.step)},execution:{accepted:null,attemptCount:null,replayed:true,dispatched:null,completed:null,commandId:null,plannedLowLevelSteps:null,completedLowLevelSteps:null,error:technicalError(errorWithCode('body_step_outcome_unknown','step was durably reserved by an earlier execution context; BODY cannot prove whether it was accepted, attempted, dispatched, or completed, and will not execute it again'))},observation:{before:null,after:null,changes:null}};}
