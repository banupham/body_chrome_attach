...
function pairingConsoleCommand(auth,line,{disconnectExtension=()=>false}={}){
  ...
  if(action==='forget'){
    ...
    return {handled:true,result:{extensionId,forgotten,disconnected,willAutoPairOnReconnect:forgotten}};
  }
  throw new Error('pair_usage: pair status | pair list | pair forget <extensionId>');
}
...
function pairedList(auth){
  return Object.entries(auth?.extensions||{}).map(([extensionId,record])=>({
    ...
    pairingMode:record.pairingMode||'legacy'
