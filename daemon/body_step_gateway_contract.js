...
test('durably RESERVED step from an earlier execution context is never executed again',async()=>{
  const baseDir=tmp('body-step-reserved'),runtime=runtimeStub(),command=validateBodyStepCommand({contractVersion:'1.0',type:'BODY_STEP',stepId:'STEP-UNKNOWN',taskId:'TASK-1',step:{kind:'motor',intent:{type:'click',x:1,y:2}}});const ledger=new BodyStepLedger(baseDir);ledger.reserve(command);const body=gateway(runtime,{baseDir});const result=await body.execute({...command,requestId:'retry'});assert.equal(result.execution.replayed,true);assert.equal(result.execution.attemptCount,0);assert.equal(result.execution.error.code,'body_step_outcome_unknown');assert.equal(runtime.calls.motor,0);
});
