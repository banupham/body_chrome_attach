...
test('BODY Contract v1 validates one atomic Body step and whitelists browser UI actions',()=>{
  ...
  assert.throws(()=>validateBodyStepCommand({contractVersion:'1.0',type:'BODY_STEP',stepId:'STEP-6',taskId:'TASK-1',step:{kind:'browser_ui',action:'internal_magic'}}),/body_browser_action_unsupported/);
...
test('Eyes never converts missing optional integers into invented zero values',async()=>{
  const runtime=runtimeStub();delete runtime.requestExtension;const browser=runtime.browsers.require('browser-a');browser.activeTabId=null;browser.tabs.get(1).windowId=null;browser.tabs.get(1).navigationEpoch=null;const body=gateway(runtime,{now:()=>1000});const observation=await body.observe({browserInstanceId:'browser-a',tabId:1});assert.equal(observation.scope.windowId,null);assert.equal(observation.scope.navigationEpoch,null);assert.equal(observation.bodyState.activeTabId,null);assert.equal(observation.freshness.liveRefreshAttempted,false);assert.equal(observation.freshness.liveRefreshSucceeded,false);
});

test('one BODY_STEP invokes one motor execution, requires a RUNNING Task, and strips task judgment fields',async()=>{
  const runtime=runtimeStub(),body=gateway(runtime,{now:()=>1200});const result=await body.execute({contractVersion:'1.0',type:'BODY_STEP',stepId:'STEP-1',taskId:'TASK-1',step:{kind:'motor',intent:{type:'click',x:10,y:20}}});assert.equal(runtime.calls.contexts,1);assert.deepEqual(runtime.calls.lastContextOptions,{autoStart:false});assert.equal(runtime.calls.motor,1);assert.equal(result.type,'BODY_STEP_RESULT');assert.equal(result.execution.accepted,true);assert.equal(result.execution.attemptCount,1);assert.equal(result.execution.replayed,false);assert.equal(result.execution.dispatched,true);assert.equal(result.execution.completed,true);assert.equal(result.execution.plannedLowLevelSteps,3);assert.equal(result.execution.completedLowLevelSteps,3);assert.equal(result.observation.changes.changed,true);assert.equal(judgmentPaths(result).length,0);assert.equal(JSON.stringify(result).includes('private-motor-detail'),false);
});
...
test('technical execution error reports unknown dispatch truth and never retries',async()=>{
  const runtime=runtimeStub(),body=gateway(runtime);const result=await body.execute({contractVersion:'1.0',type:'BODY_STEP',stepId:'STEP-FAIL',taskId:'TASK-1',step:{kind:'motor',intent:{type:'pressKey',key:'FAIL'}}});assert.equal(runtime.calls.motor,1);assert.equal(result.execution.accepted,true);assert.equal(result.execution.attemptCount,1);assert.equal(result.execution.dispatched,null);assert.equal(result.execution.completed,false);assert.equal(result.execution.error.code,'physical_dispatch_error');assert.equal(judgmentPaths(result).length,0);
});

test('invalid/rejected command does not touch physical runtime',async()=>{
  const runtime=runtimeStub(),body=gateway(runtime);const result=await body.execute({contractVersion:'1.0',type:'BODY_STEP',stepId:'STEP-X',taskId:'TASK-1',step:{kind:'motor',intent:{type:'strategy'}}});assert.equal(result.execution.accepted,false);assert.equal(result.execution.attemptCount,0);assert.equal(runtime.calls.contexts,0);assert.equal(runtime.calls.motor,0);
});

test('browser UI and tab switch remain one Body step each',async()=>{
  const runtime=runtimeStub(),body=gateway(runtime);const browser=await body.execute({contractVersion:'1.0',type:'BODY_STEP',stepId:'STEP-B',taskId:'TASK-1',step:{kind:'browser_ui',action:'back'}});assert.equal(runtime.calls.browser,1);assert.equal(browser.execution.attemptCount,1);assert.equal(browser.execution.completed,true);assert.equal(judgmentPaths(browser).length,0);const switched=await body.execute({contractVersion:'1.0',type:'BODY_STEP',stepId:'STEP-T',taskId:'TASK-1',step:{kind:'tab_switch',targetTabId:2}});assert.equal(runtime.calls.switch,1);assert.equal(switched.action.name,'switchTab');assert.equal(switched.execution.completed,true);assert.equal(judgmentPaths(switched).length,0);
});

test('duplicate BODY_STEP is replayed without a second physical execution, including after gateway restart',async()=>{
  const baseDir=tmp('body-step-idempotent'),runtime=runtimeStub();let body=gateway(runtime,{baseDir});
  const command={contractVersion:'1.0',type:'BODY_STEP',requestId:'req-1',stepId:'STEP-IDEM',taskId:'TASK-1',step:{kind:'motor',intent:{type:'click',x:10,y:20}}};
  const first=await body.execute(command);assert.equal(first.execution.replayed,false);assert.equal(runtime.calls.motor,1);
  const duplicate=await body.execute({...command,requestId:'req-2'});assert.equal(duplicate.execution.replayed,true);assert.equal(runtime.calls.motor,1);
  body=gateway(runtime,{baseDir});const afterRestart=await body.execute({...command,requestId:'req-3'});assert.equal(afterRestart.execution.replayed,true);assert.equal(runtime.calls.motor,1);
});

test('same taskId/stepId with a different command is rejected as conflict',async()=>{
  const runtime=runtimeStub(),body=gateway(runtime);const base={contractVersion:'1.0',type:'BODY_STEP',stepId:'STEP-CONFLICT',taskId:'TASK-1'};await body.execute({...base,step:{kind:'motor',intent:{type:'pressKey',key:'Enter'}}});assert.equal(runtime.calls.motor,1);const conflict=await body.execute({...base,step:{kind:'motor',intent:{type:'pressKey',key:'Escape'}}});assert.equal(conflict.execution.attemptCount,0);assert.equal(conflict.execution.error.code,'body_step_id_conflict');assert.equal(runtime.calls.motor,1);
});

test('durably RESERVED step from an earlier execution context is never executed again and preserves unknown truth',async()=>{
  const baseDir=tmp('body-step-reserved'),runtime=runtimeStub(),command=validateBodyStepCommand({contractVersion:'1.0',type:'BODY_STEP',stepId:'STEP-UNKNOWN',taskId:'TASK-1',step:{kind:'motor',intent:{type:'click',x:1,y:2}}});const ledger=new BodyStepLedger(baseDir);ledger.reserve(command);const body=gateway(runtime,{baseDir});const result=await body.execute({...command,requestId:'retry'});assert.equal(result.execution.replayed,true);assert.equal(result.execution.accepted,null);assert.equal(result.execution.attemptCount,null);assert.equal(result.execution.dispatched,null);assert.equal(result.execution.completed,null);assert.equal(result.execution.error.code,'body_step_outcome_unknown');assert.equal(runtime.calls.motor,0);
});
