  ...
  ws.on('message',async raw=>{
    ...
    if(msg.type==='HELLO'){
      ...
      if(requestedRole==='extension'){
        ...
        printAsync(`[ONLINE] extension=${extId} tabs=${item.tabs.size} auth=${authResult.paired?'paired':'verified'}`);runtime.requestExtension(extId,'RECORD_SET',{enabled:runtime.recordingEnabled}).catch(()=>{});updatePrompt();brainSend('BODY_EVENT',{event:{eventType:'extensionOnline',extensionId:extId,tabCount:item.tabs.size,ts:Date.now()}});return;
      }
      if(requestedRole==='brain'){
        ...
        role='brain';runtime.send(ws,{type:'HELLO_ACK',role:'brain',protocolVersion:PROTOCOL_VERSION,authenticated:true,controller:controller.status()});printAsync('[BRAIN] controller attached');return;
      }
      if(requestedRole==='debug_client'){
        if(!auth.authenticateDebugClient(msg.token)){rejectAuth(ws,'debug_client_token_invalid');return;}role='debug_client';debugClients.add(ws);runtime.send(ws,{type:'HELLO_ACK',role:'debug_client',protocolVersion:PROTOCOL_VERSION,authenticated:true,controller:controller.status()});return;
      }
    ...
    if(role==='extension'){
      ...
      if(msg.type==='TAB_REMOVED'){runtime.tabRemoved(extId,msg.tabId);brainSend('BODY_EVENT',{event:{eventType:'tabRemoved',extensionId:extId,tabId:msg.tabId,ts:Date.now()}});return;}return;
    }
    if(role==='brain'){
    ...
    if(role==='debug_client'){
      ...
      try{const result=await router.runCommand(msg.command,{assertControl:()=>controller.assertDebugControlAllowed()});runtime.send(ws,{type:'COMMAND_RESULT',requestId:msg.requestId||null,ok:true,result});}
      catch(error){runtime.send(ws,{type:'CLIENT_ERROR',requestId:msg.requestId||null,ok:false,error:String(error?.message||error)});}return;
    }
  });
  ws.on('close',()=>{
    const extId=runtime.registry.unregisterSocket(ws);if(extId){printAsync(`[OFFLINE] extension=${extId}`);updatePrompt();brainSend('BODY_EVENT',{event:{eventType:'extensionOffline',extensionId:extId,ts:Date.now()}});}
...
console.log('daemon.cmd is the Body runtime. The CMD prompt is diagnostic/test only; Brain is the exclusive controller when attached.');
rl=readline.createInterface({input:process.stdin,output:process.stdout,prompt:prompt()});rl.prompt();
rl.on('line',async line=>{try{const out=await router.runCommand(line,{assertControl:()=>controller.assertDebugControlAllowed()});if(out!==null)console.log(typeof out==='string'?out:JSON.stringify(out,null,2));}catch(error){console.log('[LỖI]',String(error?.message||error));}updatePrompt();rl.prompt();});

module.exports={runtime,router,wss,auth,controller,handleBrainMessage};
