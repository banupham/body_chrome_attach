...
wss.on('connection',(ws,request)=>{
  ...
  ws.on('message',async raw=>{
    ...
    if(role==='extension'){
      ...
      if(msg.type==='TAB_EVENT'){bodyGateway.observeTabEvent(extId,msg.event||{});runtime.tabEvent(extId,msg.event||{});const event={...msg.event,identity:runtime.identityForExtension(extId),browserInstanceId:item.browserInstanceId};brainSend('BODY_EVENT',{event});guardianSend('GUARDIAN_EVENT',{event});sendReadiness(ws,item.browserInstanceId);return;}
      if(msg.type==='TAB_CONTEXT'){bodyGateway.observeTabContext(extId,msg);runtime.tabContext(extId,msg);const event={eventType:'tabContext',identity:runtime.identityForExtension(extId),browserInstanceId:item.browserInstanceId,tabId:msg.tabId,context:msg.context,ts:Date.now()};brainSend('BODY_EVENT',{event});guardianSend('GUARDIAN_EVENT',{event});sendReadiness(ws,item.browserInstanceId);return;}
      if(msg.type==='TAB_REMOVED'){bodyGateway.clearTab(item.browserInstanceId,msg.tabId);runtime.tabRemoved(extId,msg.tabId);const event={eventType:'tabRemoved',identity:runtime.identityForExtension(extId),browserInstanceId:item.browserInstanceId,tabId:msg.tabId,ts:Date.now()};brainSend('BODY_EVENT',{event});guardianSend('GUARDIAN_EVENT',{event});sendReadiness(ws,item.browserInstanceId);return;}
      return;
    }
