...
const runtime=createDaemonRuntime({baseDir:__dirname,printAsync}),router=createCommandRouter(runtime,{updatePrompt}),auth=new LocalAuth(__dirname),controller=new ControllerLease(),debugClients=new Set();
...
function disconnectExtensionForRevocation(extensionId){const item=runtime.registry.get(extensionId);if(!item?.online||!item.ws)return false;try{item.ws.close(1008,'Pairing revoked');return true;}catch{return false;}}
...
rl=readline.createInterface({input:process.stdin,output:process.stdout,prompt:prompt()});rl.prompt();rl.on('line',async line=>{try{const pairing=pairingConsoleCommand(auth,line,{disconnectExtension:disconnectExtensionForRevocation});const out=pairing.handled?pairing.result:await router.runCommand(line,{assertControl:()=>controller.assertDebugControlAllowed()});if(out!==null)console.log(typeof out==='string'?out:JSON.stringify(out,null,2));}catch(error){console.log('[LỖI]',String(error?.message||error));}updatePrompt();rl.prompt();});
module.exports={runtime,router,wss,auth,controller,handleBrainMessage,CONTROL_PROTOCOL_VERSION,EXTENSION_PROTOCOL_VERSIONS,protocolAllowed,browserIdFromHello,requireTaskId,requireBrowserId,disconnectExtensionForRevocation,flushStores};
