...
wss.on('connection',(ws,request)=>{
  ...
  ws.on('message',async raw=>{
    ...
    if(role==='extension'){
      ...
      if(msg.type==='RECORDER_EVENT'){runtime.recorderEvent(extId,msg);sendReadiness(ws,item.browserInstanceId);return;}
      if(msg.type==='SEMANTIC_OBSERVATION'){runtime.semanticObservation(extId,msg);return;}
      if(msg.type==='TAB_EVENT'){runtime.tabEvent(extId,msg.event||{});brainSend('BODY_EVENT',{event:{...msg.event,identity:runtime.identityForExtension(extId),browserInstanceId:item.browserInstanceId}});sendReadiness(ws,item.browserInstanceId);return;}
      if(msg.type==='TAB_CONTEXT'){runtime.tabContext(extId,msg);brainSend('BODY_EVENT',{event:{eventType:'tabContext',identity:runtime.identityForExtension(extId),browserInstanceId:item.browserInstanceId,tabId:msg.tabId,context:msg.context,ts:Date.now()}});sendReadiness(ws,item.browserInstanceId);return;}
      if(msg.type==='TAB_REMOVED'){runtime.tabRemoved(extId,msg.tabId);brainSend('BODY_EVENT',{event:{eventType:'tabRemoved',identity:runtime.identityForExtension(extId),browserInstanceId:item.browserInstanceId,tabId:msg.tabId,ts:Date.now()}});sendReadiness(ws,item.browserInstanceId);return;}
      return;
    }
...
module.exports={runtime,router,debugAdapter,wss,auth,controller,runtimeLock,handleBrainMessage,CONTROL_PROTOCOL_VERSION,EXTENSION_PROTOCOL_VERSIONS,protocolAllowed,browserIdFromHello,requireTaskId,requireBrowserId,guardianOutcomeLabel,sendReadiness,disconnectExtensionForRevocation,flushStores,clearEndpoint};
