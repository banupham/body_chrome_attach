...
const {acquireRuntimeLock,startRuntimeLockHeartbeat,releaseRuntimeLock,publishRuntimeEndpoint,clearRuntimeEndpoint}=require('./src/runtime_endpoint');
...
const runtimeLock=acquireRuntimeLock(__dirname),stopRuntimeLockHeartbeat=startRuntimeLockHeartbeat(__dirname,runtimeLock);
...
let wss;try{wss=new WebSocketServer({host:LISTEN_HOST,port:0,verifyClient:({origin},done)=>!origin||origin.startsWith('chrome-extension://')?done(true):done(false,403,'Forbidden Origin')});}catch(error){stopRuntimeLockHeartbeat();releaseRuntimeLock(__dirname,{pid:process.pid,ownerToken:runtimeLock.ownerToken});throw error;}
...
wss.once('listening',()=>{
  try{
    ...
    runtimeEndpoint=publishRuntimeEndpoint(__dirname,port,{ownerToken:runtimeLock.ownerToken});
...
function clearEndpoint(){stopRuntimeLockHeartbeat();let endpoint=false,lock=false;try{endpoint=clearRuntimeEndpoint(__dirname,{pid:process.pid,ownerToken:runtimeLock.ownerToken});}catch{}try{lock=releaseRuntimeLock(__dirname,{pid:process.pid,ownerToken:runtimeLock.ownerToken});}catch{}return {endpoint,lock};}
