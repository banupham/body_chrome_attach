...
const coldTask=planner.inferTask(target,{...queryPlan,fingerprint:{primaryTopic:'real_estate'}},accountContext),searchCandidate={videoId:'neighbor001',surface:'search_results',position:2,targetMatch:false,targetProximity:0.64,visible:true,actionRect:{x:80,y:180,width:360,height:200,centerX:260,centerY:280},mediaFormat:{kind:'UNKNOWN'},topic:'real_estate',isRadio:false},searchWorld={currentIsTarget:false,formatMismatch:false,targetVisible:null,current:{pageType:'search',videoId:null,topic:'unknown',mediaFormat:{kind:'UNKNOWN'}},candidates:[searchCandidate],history:[],advertising:{playingAd:false},tabs:[{id:1,siteKey:'youtube.com'}],tabId:1,controls:{homeLink:{}},viewport:{width:1000,height:700},body:{pointer:{known:true,x:50,y:50}},environment:{online:true},affordances:[]};
let coldPlan=planner.generate(searchWorld,{task:coldTask,queryPlan,dynamicQueries:[],usedQueries:new Set(),stagnation:0});const previewAction=coldPlan.actions.find(row=>row.type==='preview_candidate'&&row.target?.videoId==='neighbor001');assert.ok(previewAction);assert.equal(previewAction.purpose,'seed_personalization_with_confirmed_preview');assert.equal(coldPlan.subgoal.id,'seed_personalization_with_preview');assert.ok(previewAction.utility>coldPlan.actions.find(row=>row.type==='click_candidate'&&row.target?.videoId==='neighbor001').utility);
coldTask.accountContext.runtime.homeCheckDue=true;coldPlan=planner.generate(searchWorld,{task:coldTask,queryPlan,dynamicQueries:[],usedQueries:new Set(),stagnation:0});const homeCheck=coldPlan.actions.find(row=>row.type==='home');assert.ok(homeCheck);assert.equal(homeCheck.purpose,'measure_home_after_preview');assert.equal(coldPlan.subgoal.id,'measure_personalization_home');
