...
(async()=>{
  ...
  for(const required of ['ensureContentScript','repairOpenWebTabs','CONTENT_SCRIPT_FILE','files: [CONTENT_SCRIPT_FILE]','chrome.windows?.onFocusChanged','contextSource: \'chrome_window_focus\'','pendingUserMotorByTab','forwardUserMotorReliable','learningInput: learningInputStatus()'])assert.ok(serviceWorker.includes(required),`multi-Chrome learning repair missing: ${required}`);
  ...
  const popup=fs.readFileSync(path.join(__dirname,'..','src','pairing_popup.js'),'utf8');assert.ok(popup.includes('Học: ${events} sự kiện'));assert.ok(popup.includes('browserInstanceId'));assert.ok(popup.includes('activeTabId'));
