...
(async()=>{
  ...
  const content=fs.readFileSync(path.join(__dirname,'..','src','virtual_cursor_content.js'),'utf8');assert.ok(content.includes('body.environmentObservation'));assert.ok(content.includes("action:'body.pageContext'"));assert.ok(content.includes('window.location.href'));assert.equal(content.includes('navigator.webdriver ='),false);
  const worker=fs.readFileSync(path.join(__dirname,'..','src','service_worker_entry.js'),'utf8');assert.ok(worker.includes("message?.action === 'body.pageContext'"));assert.ok(worker.includes("contextSource: 'content_script'"));
