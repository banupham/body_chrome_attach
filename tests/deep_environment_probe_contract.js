...
const {analyzeDeepSnapshots,deepEnvironmentProbe,collectRealmSnapshot}=require('../src/deep_environment_probe');
...
(async()=>{
  ...
  const injectedSource=collectRealmSnapshot.toString();assert.equal(injectedSource.includes('AUTOMATION_MARKERS'),false,'injected function must not reference module-scope AUTOMATION_MARKERS');assert.ok(injectedSource.includes('automationMarkers'));
