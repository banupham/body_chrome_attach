...
function hydrationEvidence(obs){
  ...
  const uiReady=routeReady&&(searchControlReady||surfaceRootReady||candidateReady||String(obs?.signedInState||'unknown')!=='unknown');
  return {pageType,routeReady,searchControlReady,surfaceRootReady,candidateReady,uiReady};
}
function pristineSnapshot(obs,tabs){
  const allTabs=Array.isArray(tabs)?tabs:[],yt=youtubeTabs(allTabs),signedInState=String(obs?.signedInState||'unknown'),readiness=hydrationEvidence(obs);
  return {signedInState,tabCount:allTabs.length,youtubeTabCount:yt.length,shapeOk:allTabs.length<=2&&yt.length===1,readiness};
}
function pristineDecision(snapshot,{stableSignedOutSamples=0,requiredStableSamples=2,requested=true}={}){
  ...
  if(signed==='signed_in')return {status:'fail',reason:'signed_in_detected',stableSignedOutSamples:0};
  ...
  async validatePristine(){
    ...
    while(true){
      ...
      if(lastDecision.status==='pass'){
        this.pristineEvidence={requested,pass:true,reason:lastDecision.reason,signedInState:lastSnapshot.signedInState,tabCount:lastSnapshot.tabCount,youtubeTabCount:lastSnapshot.youtubeTabCount,pageType:lastSnapshot.readiness.pageType,readiness:lastSnapshot.readiness,attempts,waitedMs,stableSignedOutSamples,requiredStableSignedOutSamples:requiredStableSamples};
      ...
      this.log('pristine_browser_wait',{reason:lastDecision.reason,attempt:attempts,waitedMs,signedInState:lastSnapshot.signedInState,stableSignedOutSamples,requiredStableSignedOutSamples:requiredStableSamples,readiness:lastSnapshot.readiness});
      if(waitedMs>=timeoutMs){
        this.pristineEvidence={requested,pass:false,reason:'signed_in_state_unresolved_after_settle',signedInState:lastSnapshot.signedInState,tabCount:lastSnapshot.tabCount,youtubeTabCount:lastSnapshot.youtubeTabCount,pageType:lastSnapshot.readiness.pageType,readiness:lastSnapshot.readiness,attempts,waitedMs,stableSignedOutSamples,requiredStableSignedOutSamples:requiredStableSamples};
        this.log('pristine_browser_check',this.pristineEvidence);
