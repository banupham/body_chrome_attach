...
function parseArgs(argv=process.argv.slice(2)){
  const out={target:null,selftest:false,unlimited:false,continueAfterFound:false,maxMinutes:30,maxSteps:120,maxQueries:18,maxDwellSec:90,reportEverySteps:10,reportEveryMinutes:10,browser:null,tab:null,root:null};
...
async function main(){
  ...
  console.log('BODY Autonomous YouTube Discovery Brain');console.log(`Target videoId: ${config.target}`);console.log(`Mode: ${config.unlimited?'UNLIMITED':'BOUNDED'} | continueAfterFound=${config.continueAfterFound}`);console.log('YouTube API key: read from YOUTUBE_DATA_API_KEY / YOUTUBE_API_KEY (never written to reports).');
