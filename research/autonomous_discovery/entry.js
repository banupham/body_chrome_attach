...
const {AutonomousYouTubeBrainV4}=require('./brain_v4');
...
const {normalizeMode,normalizeSurface}=require('./surface_policy');
...
function parseArgs(argv=process.argv.slice(2)){
  const out={target:null,accountId:null,discoveryMode:'auto',discoverySurface:'auto',selftest:false,unlimited:false,continueAfterFound:false,maxMinutes:30,maxSteps:120,maxQueries:24,maxDwellSec:90,reportEverySteps:10,reportEveryMinutes:10,browser:null,tab:null,root:null,browserWaitSec:120,actionRetries:3,verifyTimeoutMs:5500,explorationRate:0.18};
  ...
  if(!out.target)throw new Error('usage: YouTubeExplorerBrain --target VIDEO_ID [--account-id LOCAL_ACCOUNT_ID] [--discovery-mode auto|diversity|surface_test] [--discovery-surface auto|home|search|related|mix] [--unlimited true]');
  ...
  out.discoveryMode=normalizeMode(out.discoveryMode);out.discoverySurface=normalizeSurface(out.discoverySurface);
...
function runSelftest(){
  ...
  if(normalizeMode('surface-test')!=='surface_test'||normalizeSurface('next-video')!=='related')throw new Error('selftest_surface_policy_normalization_failed');
  console.log('YouTubeExplorerBrain selftest: PASS');return 0;
}

async function main(){
  const config=parseArgs();if(config.selftest)return runSelftest();
  console.log('BODY Autonomous YouTube Discovery Brain V4 Cold-Start Account Aware');console.log(`Target videoId: ${config.target}`);console.log(`Account context: ${config.accountId?`ENABLED (${config.accountId})`:'ANONYMOUS / NOT PROVIDED'}`);console.log(`Discovery mode requested: ${config.discoveryMode} | surface=${config.discoverySurface}`);console.log(`Mode: ${config.unlimited?'UNLIMITED':'BOUNDED'} | continueAfterFound=${config.continueAfterFound}`);console.log(`Agent exploration rate: ${config.explorationRate}`);console.log(`Step verification: retries=${config.actionRetries} | timeout=${config.verifyTimeoutMs}ms | browserWait=${config.browserWaitSec}s`);console.log('Autonomy: FULL_BODY_AGENT. Account-aware planning can distinguish established, cold-start, empty, unavailable-history, and signed-out states.');console.log('Interaction learning: Search, Home, related videos, topic filters, Mix and confirmed hover previews are competing observed routes; no automatic Search->Preview->Home sequence is imposed in auto mode.');console.log('Preview exposure: Brain records actual preview playback time and learns recommendation effects from observed candidate-distribution changes; it does not assume a YouTube watch threshold.');console.log('Discovery modes: auto chooses from observed routes; diversity/surface_test remain explicit diagnostic modes.');console.log('Task constraint retained: target ID/title are for profiling and success verification, not direct YouTube search or preview seeding.');console.log('Account ID is a local logical binding. It never logs into Google and does not capture Google account identity; the attached Chrome profile must already be signed in.');console.log('YouTube API key: read from YOUTUBE_DATA_API_KEY / YOUTUBE_API_KEY (never written to reports).');
  const brain=new AutonomousYouTubeBrainV4(config);let stopping=false;
  ...
  try{const report=await brain.run();console.log('\n=== DISCOVERY FINISHED ===');console.log(JSON.stringify({runId:report.runId,status:report.status,steps:report.summary.steps,targetDiscovery:report.targetDiscovery,discoveryPolicy:report.discoveryPolicy?{mode:report.discoveryPolicy.mode,preferredSurface:report.discoveryPolicy.preferredSurface,fallbackActive:report.discoveryPolicy.fallbackActive}:null,account:report.account?{accountKey:report.account.accountKey,accountState:report.account.accountState,relationship:report.account.relationship?.kind,historySampleCount:report.account.historySampleCount,homeSampleCount:report.account.homeSampleCount,previewConfirmedCount:report.account.brainActivity?.previewConfirmedCount||0,personalizationSeeded:report.account.runtime?.personalizationSeeded===true}:null,reportsRoot:brain.reportDir,sessionLedger:brain.ledgerFile,memoryFile:brain.memory.file,planner:report.autonomy?.planner},null,2));return report;}
