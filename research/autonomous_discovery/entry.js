...
async function main(){
  ...
  console.log('BODY Autonomous YouTube Discovery Brain V4 Cold-Start Account Aware');console.log(`Target videoId: ${config.target}`);console.log(`Account context: ${config.accountId?`ENABLED (${config.accountId})`:'ANONYMOUS / NOT PROVIDED'}`);console.log(`Discovery mode requested: ${config.discoveryMode} | surface=${config.discoverySurface}`);console.log(`Mode: ${config.unlimited?'UNLIMITED':'BOUNDED'} | continueAfterFound=${config.continueAfterFound}`);console.log(`Agent exploration rate: ${config.explorationRate}`);console.log(`Step verification: retries=${config.actionRetries} | timeout=${config.verifyTimeoutMs}ms | browserWait=${config.browserWaitSec}s`);console.log('Autonomy: FULL_BODY_AGENT. Account-aware planning can distinguish established, cold-start, empty, unavailable-history, and signed-out states.');console.log('Cold-start behavior: target-derived Search can seed YouTube personalization with confirmed non-target hover previews, then Home is measured before the planner continues.');console.log('Discovery modes: auto chooses freely; diversity softly favors underused Home/Search/Related/Mix routes; surface_test strongly tests one requested route with a bounded fallback.');console.log('Task constraint retained: target ID/title are for profiling and success verification, not direct YouTube search or preview seeding.');console.log('Account ID is a local logical binding. It never logs into Google and does not capture Google account identity; the attached Chrome profile must already be signed in.');console.log('YouTube API key: read from YOUTUBE_DATA_API_KEY / YOUTUBE_API_KEY (never written to reports).');
