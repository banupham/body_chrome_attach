...
class AccountProfileStore{
  ...
  recordActivity(event){if(!this.activityPath)return null;const state=this.loadActivity(),row={at:new Date().toISOString(),type:clean(event?.type)||'UNKNOWN',videoId:clean(event?.videoId)||null,surface:clean(event?.surface)||null,targetMatch:event?.targetMatch===true,previewConfirmed:event?.previewConfirmed===true,behavioralExposureConfirmed:event?.behavioralExposureConfirmed===true,hoverMs:Number(event?.hoverMs||0)||0,previewPlaybackDeltaSec:Number(event?.previewPlaybackDeltaSec||0)||0,query:event?.query?String(event.query).slice(0,300):null,generatedByBrain:true};state.events.push(row);if(state.events.length>2000)state.events.splice(0,state.events.length-2000);state.updatedAt=new Date().toISOString();fs.mkdirSync(this.root,{recursive:true});atomicWrite(this.activityPath,state);return row;}
