...
function affordanceActions(world,{textProbe=null,targetVocabulary=[],targetFormat=FORMAT.UNKNOWN}={}){
  ...
  for(const aff of (world.affordances||[]).slice(0,100)){
    if(aff.disabled||!aff.actionRect)continue;const center=aff.interactionPoint||rectCenter(aff.actionRect);if(!center)continue;const linkPath=String(aff.link?.path||''),shortLink=aff.link?.youtube===true&&/^\/shorts\//i.test(linkPath);if(targetKind===FORMAT.LONG_FORM&&shortLink)continue;
