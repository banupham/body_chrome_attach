...
const {AutonomousYouTubeBrainV3Recovery}=require('./brain_v3_recovery');
...
const {SurfacePolicyStore,MODE}=require('./surface_policy');
...
class AutonomousYouTubeBrainV4 extends AutonomousYouTubeBrainV3Recovery{
  constructor(config,deps={}){
    super(config,deps);this.autonomyMode='full_body_agent_causal_interaction_learning';this.previewExposureCount=0;this.previewConfirmedCount=0;this.searchImpressionCount=0;this.searchImpressionIds=new Set();this.surfaceDiscoveryRecorded=false;this.recentPreviewExposure=null;
    const surfaceRoot=this.accountProfileStore?.root||this.root;this.surfacePolicyStore=new SurfacePolicyStore(surfaceRoot);this.discoveryPolicy=this.surfacePolicyStore.beginRun({targetVideoId:config.target,mode:config.discoveryMode,requestedSurface:config.discoverySurface});
    const baseInfer=this.agentPlanner.inferTask.bind(this.agentPlanner);this.agentPlanner.inferTask=(targetProfile,queryPlan,accountContext)=>baseInfer(targetProfile,queryPlan,accountContext,this.discoveryPolicy);
    this.ledger('discovery_surface_policy_started',{mode:this.discoveryPolicy.mode,requestedSurface:this.discoveryPolicy.requestedSurface,preferredSurface:this.discoveryPolicy.preferredSurface,fallbackAfterSteps:this.discoveryPolicy.fallbackAfterSteps,prior:this.discoveryPolicy.prior,storeFile:this.discoveryPolicy.storeFile});
  }
  ...
  async worldFrom(state,snapshotInfo){
    const world=await super.worldFrom(state,snapshotInfo);world.recentExposure=this.recentPreviewExposure?{...this.recentPreviewExposure,ageSteps:Math.max(0,this.plannerDecisionCount-Number(this.recentPreviewExposure.atDecision||0))}:null;return world;
  }
  ...
  async previewCandidate(action,snapshotInfo){
    const selected=snapshotInfo.snapshot.candidates.find(c=>String(c.videoId)===String(action.target?.videoId))||null;if(!selected)return {success:false,result:null,query:null,selected:null,dwellSec:0,error:'planned_preview_candidate_not_in_snapshot',previewConfirmed:false,behavioralExposureConfirmed:false,previewPlaybackDeltaSec:0};if(String(selected.videoId)===String(this.config.target))return {success:false,result:null,query:null,selected,dwellSec:0,error:'target_cannot_be_used_as_preview_seed',previewConfirmed:false,behavioralExposureConfirmed:false,previewPlaybackDeltaSec:0};if(!selected.actionRect)return {success:false,result:null,query:null,selected,dwellSec:0,error:'preview_candidate_has_no_action_rect',previewConfirmed:false,behavioralExposureConfirmed:false,previewPlaybackDeltaSec:0};
    const r=selected.actionRect,x=Number(r.centerX??(Number(r.x||0)+Number(r.width||0)/2)),y=Number(r.centerY??(Number(r.y||0)+Number(r.height||0)/2)),started=Date.now();let hoverResult=null,startState=null,progressState=null,finalState=null,error=null,confirmed=false,behavioralExposureConfirmed=false,previewPlaybackDeltaSec=0;
    try{
      hoverResult=await this.motor({type:'hover',x,y,width:Number(r.width||12),height:Number(r.height||12),role:'link'});
      startState=await this.waitForSemantic(s=>Boolean(s?.preview?.active&&s?.preview?.playing&&String(s?.preview?.videoId||'')===String(selected.videoId)),{timeoutMs:4200,intervalMs:180,reason:'preview_start_verify'});
      if(!startState?.semantic?.preview?.active)throw new Error('hover_preview_did_not_start');
      const startTime=Number(startState.semantic.preview.currentTime||0),holdMs=randomInt(4500,11000),probeMs=Math.min(1800,Math.max(1000,Math.floor(holdMs*0.28)));
      await sleep(probeMs);progressState=await this.observe('preview_progress_verify');const progress=progressState?.semantic?.preview;confirmed=Boolean(progress?.active&&progress?.playing&&String(progress?.videoId||'')===String(selected.videoId)&&Number(progress.currentTime||0)>=startTime+0.45);if(!confirmed)throw new Error('hover_preview_playback_not_confirmed');
      if(holdMs>probeMs)await sleep(holdMs-probeMs);finalState=await this.observe('preview_exposure_final_verify');const finalPreview=finalState?.semantic?.preview,finalTime=String(finalPreview?.videoId||'')===String(selected.videoId)?Number(finalPreview?.currentTime||0):Number(progress?.currentTime||0);previewPlaybackDeltaSec=Math.max(0,finalTime-startTime);behavioralExposureConfirmed=confirmed&&previewPlaybackDeltaSec>=3;
      const actualHoverMs=Date.now()-started;this.previewExposureCount++;this.previewConfirmedCount++;
      const topic=selected.classification?.primary||'unknown';this.recentPreviewExposure={videoId:String(selected.videoId),topic,surface:selected.surface,playbackDeltaSec:Number(previewPlaybackDeltaSec.toFixed(3)),behavioralExposureConfirmed,at:Date.now(),atDecision:this.plannerDecisionCount,effectId:action.effectId||action.routeFamily||null};
      const runtime=this.accountContext?.runtime;if(runtime){runtime.previewExposureCount=Number(runtime.previewExposureCount||0)+1;runtime.previewConfirmedCount=Number(runtime.previewConfirmedCount||0)+1;runtime.previewedVideoIds=[...new Set([...(runtime.previewedVideoIds||[]),String(selected.videoId)])];runtime.lastPreviewExposure=this.recentPreviewExposure;}
      this.recordBrainActivity({type:'PREVIEW_EXPOSURE',videoId:selected.videoId,surface:selected.surface,targetMatch:false,previewConfirmed:true,behavioralExposureConfirmed,hoverMs:actualHoverMs,previewPlaybackDeltaSec});
      this.ledger('preview_exposure',{videoId:selected.videoId,surface:selected.surface,targetMatch:false,targetProximity:Number(selected.targetProximity||0),topic,previewConfirmed:true,behavioralExposureConfirmed,hoverMs:actualHoverMs,previewPlaybackDeltaSec:Number(previewPlaybackDeltaSec.toFixed(3)),generatedByBrain:true});
      return {success:true,result:hoverResult,query:null,selected,dwellSec:actualHoverMs/1000,error:null,previewConfirmed:true,behavioralExposureConfirmed,hoverMs:actualHoverMs,previewPlaybackDeltaSec:Number(previewPlaybackDeltaSec.toFixed(3))};
    }catch(e){
      error=String(e?.message||e);const actualHoverMs=Date.now()-started;this.ledger('preview_exposure',{videoId:selected.videoId,surface:selected.surface,targetMatch:false,targetProximity:Number(selected.targetProximity||0),topic:selected.classification?.primary||'unknown',previewConfirmed:false,behavioralExposureConfirmed:false,hoverMs:actualHoverMs,previewPlaybackDeltaSec:Number(previewPlaybackDeltaSec.toFixed(3)),error,generatedByBrain:true});return {success:false,result:hoverResult,query:null,selected,dwellSec:actualHoverMs/1000,error,previewConfirmed:false,behavioralExposureConfirmed:false,hoverMs:actualHoverMs,previewPlaybackDeltaSec:Number(previewPlaybackDeltaSec.toFixed(3))};
    }
  }
  ...
  rewardAgent(args){const value=super.rewardAgent(args);return Number(value.toFixed(3));}
  maybeActivateSurfaceFallback(){
    const p=this.discoveryPolicy;if(!p||p.mode!==MODE.SURFACE_TEST||p.fallbackActive)return false;const threshold=Math.max(8,Number(p.fallbackAfterSteps)||25);if(this.plannerDecisionCount<threshold&&this.stagnation<8)return false;p.fallbackActive=true;this.ledger('discovery_surface_fallback_activated',{preferredSurface:p.preferredSurface,plannerDecisionCount:this.plannerDecisionCount,stagnation:this.stagnation,threshold});return true;
  }
  markTargetOpened(meta={}){
    const discovery=super.markTargetOpened(meta);if(!this.surfaceDiscoveryRecorded&&discovery?.opened){const recorded=this.surfacePolicyStore?.recordDiscovery(this.discoveryPolicy,discovery.surface);this.surfaceDiscoveryRecorded=true;this.ledger('discovery_surface_recorded',{policy:{mode:this.discoveryPolicy?.mode,preferredSurface:this.discoveryPolicy?.preferredSurface,fallbackActive:this.discoveryPolicy?.fallbackActive===true},actualSurface:discovery.surface,recorded});}return discovery;
  }
  async actAgent(state,snapshotInfo){
    this.maybeActivateSurfaceFallback();const result=await super.actAgent(state,snapshotInfo),row=this.agentHistory.at(-1);if(row?.type==='search')this.recordSearchImpressions(result?.after);return result;
  }
  reportObject(status){const report=super.reportObject(status);report.schemaVersion=9;if(report.account){report.account.accountState=this.accountContext?.accountState||null;report.account.homeSampleCount=this.accountContext?.homeSampleCount||0;report.account.baseline=this.accountContext?.baseline||null;report.account.runtime=this.accountContext?.runtime||null;report.account.activityFile=this.accountProfileStore?.activityFile()||null;report.account.brainActivity={searchImpressionCount:this.searchImpressionCount,previewExposureCount:this.previewExposureCount,previewConfirmedCount:this.previewConfirmedCount};}report.discoveryPolicy=this.discoveryPolicy?{mode:this.discoveryPolicy.mode,requestedSurface:this.discoveryPolicy.requestedSurface,preferredSurface:this.discoveryPolicy.preferredSurface,fallbackAfterSteps:this.discoveryPolicy.fallbackAfterSteps,fallbackActive:this.discoveryPolicy.fallbackActive===true,prior:this.discoveryPolicy.prior,current:this.surfacePolicyStore?.summary(this.discoveryPolicy)||null,storeFile:this.discoveryPolicy.storeFile}:null;report.autonomy={...report.autonomy,coldStartPreviewBootstrap:false,previewAsBehavioralExposure:true,previewSuccessEvidence:'observed_hover_preview_playback_progress',previewRecommendationEffect:'learned_from_observed_candidate_distribution_change',brainGeneratedActivityExcludedFromBaseline:true,activityClasses:['SEARCH_IMPRESSION','PREVIEW_EXPOSURE','OPENED_VIDEO'],multiSurfaceDiscoveryPolicy:true,discoveryMode:this.discoveryPolicy?.mode||MODE.AUTO,preferredDiscoverySurface:this.discoveryPolicy?.preferredSurface||'auto',recentPreviewExposure:this.recentPreviewExposure};return report;}
}
