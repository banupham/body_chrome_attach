...
function specificPlanningProximity(candidate,fingerprint={}){
  ...
  const targetLanguage=String(fingerprint.language||''),candidateLanguage=String(api.defaultLanguage||api.defaultAudioLanguage||'');if(targetLanguage&&candidateLanguage&&candidateLanguage.startsWith(targetLanguage))score+=0.03;
...
class AutonomousYouTubeBrainV3Recovery extends AutonomousYouTubeBrainV3{
  constructor(config,deps={}){super(config,deps);this.bestSpecificProximity=0;}
  ...
  async prepareCandidateForSafeClick(candidate,{maxScrolls=2}={}){
    for(let attempt=0;attempt<=maxScrolls;attempt++){
      ...
      await this.motor({type:'moveTo',x:mouseX,y:mouseY,role:'page'});await this.motor({type:'scrollVertical',delta});this.ledger('safe_click_reposition',{videoId:candidate.videoId,attempt:attempt+1,delta,hadCandidate:Boolean(match),rawRect:match?.actionRect||null});await sleep(260);
    }
    return null;
  }
  async clickCandidateSafe(candidate){
    ...
    const {match,safeRect,scrolls}=prepared,beforeTabs=await this.browserTabs(),sourceTabId=this.tabId,p=randomPointInRect(safeRect,{pad:8}),result=await this.motor({type:'click',x:p.x,y:p.y,width:safeRect.width,height:safeRect.height,role:'link'}),tabOutcome=await this.reconcileTabEffects(beforeTabs,{reason:'candidate_safe_click',expectedVideoId:candidate.videoId,sourceTabId});
  ...
  async executeAgentAction(action,snapshotInfo,world){
    ...
    try{selected=snapshotInfo.snapshot.candidates.find(c=>String(c.videoId)===String(action.target?.videoId))||null;if(!selected)throw new Error('planned_candidate_not_in_snapshot');const clicked=await this.clickCandidateSafe(selected);success=clicked.ok;result=clicked.result||null;if(success){this.visitedVideos.add(selected.videoId);if(String(selected.videoId)===String(this.config.target)){this.targetOpened=true;this.targetDiscovery=this.targetDiscovery||{at:Date.now(),surface:selected.surface,method:selected.surface,rank:selected.position??null,fromVideoId:world.current.videoId||null,fromTopic:world.current.topic,strategy:'agent_planner',opened:true,openedAt:Date.now(),title:selected.youtubeApi?.title||selected.title||null};}}}catch(e){error=String(e?.message||e);}
    return {success,result,query:null,selected,dwellSec:0,error,durationMs:Date.now()-started};
  }
  rewardAgent({action,outcome,delta,afterInfo}){let value=this.reward({newVideos:afterInfo.newVideos,newTopics:afterInfo.newTopics,newTransitions:afterInfo.newTransitions,proximityGain:afterInfo.proximityGain,targetSeen:Boolean(afterInfo.targetCandidate),targetOpened:this.targetOpened,success:outcome.success});if(delta.changed)value+=2;if(String(action.selection||'').startsWith('exploration')&&delta.changed)value+=1;if(!delta.changed&&outcome.success)value-=1.5;if(outcome.error)value-=4;return Number(value.toFixed(3));}
  reportObject(status){const report=super.reportObject(status);report.schemaVersion=6;if((!report.path||!report.path.length)&&this.agentHistory.length)report.path=this.agentHistory.slice(-12);if((!report.snapshots||!report.snapshots.length)&&this.lastSnapshot)report.snapshots=[this.lastSnapshot];report.autonomy={...report.autonomy,revision:'safe_click_recovery_specificity_v1',safeCandidateClick:'content_viewport_clip_single_click_v1'};report.agent={...(report.agent||{}),diagnostics:historyDiagnostics(this.agentHistory.slice(-250))};return report;}
}
