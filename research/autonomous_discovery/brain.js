...
class AutonomousYouTubeBrain{
  ...
  async search(queryRow){
    ...
    await this.motor({type:'click',x:centerX,y:centerY,width:r.width,height:r.height,role:'textbox'});await this.motor({type:'keyCombo',key:'Control+a'});await this.motor({type:'typeText',x:centerX,y:centerY,width:r.width,height:r.height,role:'textbox',text:inspected.query});await this.motor({type:'pressKey',key:'Enter'});this.queryAttempts++;this.ledger('search_submitted',{query:inspected.query,kind:queryRow.kind,titleOverlap:inspected.titleOverlap});
  ...
  async scrollToCandidate(candidate,maxScrolls=8){
    for(let i=0;i<maxScrolls;i++){
      const state=await this.observe(`rebind_${candidate.videoId}`);const semantic=state.semantic;if(!semantic)return null;const match=flattenCandidates(semantic).find(x=>String(x.videoId)===String(candidate.videoId)&&String(x.surface)===String(candidate.surface))||flattenCandidates(semantic).find(x=>String(x.videoId)===String(candidate.videoId));if(match?.visible&&match.actionRect)return match;const viewport=semantic.viewport||{width:1000,height:700},centerX=bounded(Number(viewport.width||1000)*0.55,20,Number(viewport.width||1000)-20),centerY=bounded(Number(viewport.height||700)*0.55,20,Number(viewport.height||700)-20);let delta=650;if(match?.actionRect&&Number.isFinite(Number(match.actionRect.centerY)))delta=bounded(Number(match.actionRect.centerY)-centerY,-850,850);if(Math.abs(delta)<150)delta=delta>=0?350:-350;await this.motor({type:'moveTo',x:centerX,y:centerY,role:'page'});await this.motor({type:'scrollVertical',delta});await sleep(300);
    }return null;
  }
  async clickCandidate(candidate){
    let actionable=candidate;if(!candidate.visible||!candidate.actionRect)actionable=await this.scrollToCandidate(candidate);if(!actionable?.actionRect)return {ok:false,reason:'candidate_unactionable'};const r=actionable.actionRect;const x=Number(r.centerX??(r.x+r.width/2)),y=Number(r.centerY??(r.y+r.height/2));const result=await this.motor({type:'click',x,y,width:r.width,height:r.height,role:'link'});const completed=result?.execution?.completed===true;return {ok:completed,result};
  }
  dwellSeconds(){
