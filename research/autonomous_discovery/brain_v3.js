...
const {buildWorld,worldDelta,recommendationDelta,contextKey}=require('./world_model');
...
class AutonomousYouTubeBrainV3 extends AutonomousYouTubeBrainV2{
  ...
  async actAgent(state,snapshotInfo){
    ...
    this.ledger('agent_plan',{decision:this.plannerDecisionCount,subgoal:plan.subgoal,context:plan.context,targetFormat:plan.targetFormat,currentFormat:world.current.mediaFormat,formatMismatch:world.formatMismatch,accountRelationship:plan.accountRelationship,accountPlanMode:plan.accountPlanMode,selected:{type:action.type,capability:action.capability,purpose:action.purpose,query:action.query||null,targetVideoId:action.target?.videoId||null,targetMediaFormat:action.target?.mediaFormat||null,formatRelation:action.formatRelation||null,tabId:action.tabId??null,utility:action.utility,selection:action.selection,epsilon:action.epsilon,routeFamily:action.routeFamily||null,routeFrontierAdjustment:Number(action.routeFrontierAdjustment||0),causalEffectBonus:Number(action.causalEffectBonus||0)},alternatives:plan.actions.slice(0,8).map(a=>({type:a.type,capability:a.capability,purpose:a.purpose,query:a.query||null,targetVideoId:a.target?.videoId||null,targetMediaFormat:a.target?.mediaFormat||null,formatRelation:a.formatRelation||null,utility:a.utility,routeFamily:a.routeFamily||null,routeFrontierAdjustment:Number(a.routeFrontierAdjustment||0),causalEffectBonus:Number(a.causalEffectBonus||0)}))});
    const outcome=await this.executeAgentAction(action,snapshotInfo,world);let afterState=await this.observe('agent_after_action');if(!afterState.semantic)afterState=await this.semanticRecovery();if(!afterState?.semantic)throw new Error('agent_semantic_recovery_failed');const afterInfo=await this.enrichedSnapshot(afterState,`agent_after_${action.type}`);const afterWorld=await this.worldFrom(afterState,afterInfo),delta=worldDelta(world,afterWorld),recommendations=recommendationDelta(world,afterWorld);const success=outcome.success&&(delta.changed||(action.type==='preview_candidate'&&outcome.previewConfirmed===true)||action.type==='dwell'||action.capability?.startsWith('motor.moveTo')||action.capability?.startsWith('motor.hover'));
    const reward=this.rewardAgent({action,outcome:{...outcome,success},delta,afterInfo}),memoryId=actionMemoryId(action),context=contextKey(world),effectId=action.effectId||action.routeFamily||memoryId;this.memory.recordStrategy(memoryId,{context,reward,success,newTopics:afterInfo.newTopics,newTransitions:afterInfo.newTransitions,targetSeen:Boolean(afterInfo.targetCandidate)||this.targetOpened});this.memory.recordInteractionEffect?.(effectId,{context,success,recommendationShift:recommendations.recommendationShift,proximityGain:afterInfo.proximityGain,targetSeen:Boolean(afterInfo.targetCandidate)||afterWorld.currentIsTarget,exposureSeconds:Number(outcome.previewPlaybackDeltaSec??((outcome.hoverMs||0)/1000))||0});if(outcome.query)this.memory.recordQuery(outcome.query,{reward,targetSeen:Boolean(afterInfo.targetCandidate)||this.targetOpened,resultCount:afterInfo.snapshot.candidates.length});if(!success)this.memory.addLesson('An autonomous action failed to produce its expected environment change; planner should lower this action in the same context.',{memoryId,context,error:outcome.error,delta:delta.reasons});this.memory.save();const targetProgress=success&&(Number(afterInfo.proximityGain||0)>=0.025||Boolean(afterInfo.targetCandidate&&!snapshotInfo.targetCandidate)||(afterWorld.currentIsTarget&&!world.currentIsTarget));this.stagnation=targetProgress?0:this.stagnation+1;
    const historyRow={step:this.stepNo,at:Date.now(),targetProgress,memoryId,effectId,durationMs:outcome.durationMs??outcome.hoverMs??null,previewPlaybackDeltaSec:Number(outcome.previewPlaybackDeltaSec||0),queryRepeated:outcome.queryRepeated===true,actionKey:action.actionKey,type:action.type,capability:action.capability,purpose:action.purpose,subgoal:plan.subgoal.id,accountRelationship:plan.accountRelationship,accountPlanMode:plan.accountPlanMode,targetFormat:plan.targetFormat,currentFormatBefore:world.current.mediaFormat,currentFormatAfter:afterWorld.current.mediaFormat,formatMismatchBefore:world.formatMismatch,formatMismatchAfter:afterWorld.formatMismatch,videoId:outcome.selected?.videoId||afterWorld.current.videoId||null,topic:outcome.selected?.classification?.primary||afterWorld.current.topic,query:outcome.query,tabId:this.tabId,success,reward,changed:delta.changed,changeReasons:delta.reasons,recommendationDelta:recommendations,error:outcome.error,selection:action.selection,utility:action.utility,routeFamily:action.routeFamily||null,routeFrontierAdjustment:Number(action.routeFrontierAdjustment||0),causalEffectBonus:Number(action.causalEffectBonus||0)};this.agentHistory.push(historyRow);if(this.agentHistory.length>500)this.agentHistory.splice(0,this.agentHistory.length-500);this.batchPath.push(historyRow);this.ledger('agent_outcome',historyRow);
