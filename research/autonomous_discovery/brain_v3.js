...
class AutonomousYouTubeBrainV3 extends AutonomousYouTubeBrainV2{
  ...
  reportObject(status){const report=super.reportObject(status);report.schemaVersion=5;if(report.target)report.target.mediaFormat=this.targetApi?.mediaFormat||{kind:FORMAT.UNKNOWN};report.autonomy={...report.autonomy,mode:'FULL_BODY_AGENT',planner:'world_model_affordance_utility_experience_v3',fixedStrategyMenu:false,fullBodyCapabilityCatalog:this.capabilityCatalog,taskModel:this.taskModel,languageAwareQueryPlanner:this.queryPlan?.planner||null,targetLanguage:this.queryPlan?.targetLanguage||null,targetFormat:formatKind(this.targetApi?.mediaFormat),targetFormatPolicy:this.taskModel?.formatPolicy||null};report.agent={plannerDecisions:this.plannerDecisionCount,history:this.agentHistory.slice(-250),usedQueries:[...this.usedQueries],dynamicQueries:this.dynamicQueries.slice(-50)};return report;}
  async run(){
    this.status='STARTING';this.memory.startRun();this.targetApi=await this.api.profileTarget(this.config.target);this.ledger('target_format_profiled',{videoId:this.targetApi.videoId,mediaFormat:this.targetApi.mediaFormat});this.queryPlan=adaptiveQueryPlan(this.targetApi,{maxQueries:this.config.maxQueries});this.queryPlan.fingerprint=this.buildFingerprint();this.queryAudit.push(...this.queryPlan.audit);this.taskModel=this.agentPlanner.inferTask(this.targetApi,this.queryPlan);this.ledger('target_profiled',{videoId:this.targetApi.videoId,title:this.targetApi.title,categoryId:this.targetApi.categoryId,topics:this.targetApi.topicLabels,targetLanguage:this.queryPlan.targetLanguage,targetFormat:this.taskModel.targetFormat,targetFormatConfidence:this.taskModel.targetFormatConfidence,targetFormatEvidence:this.taskModel.targetFormatEvidence,formatPolicy:this.taskModel.formatPolicy,queryPlanner:this.queryPlan.planner,safeQueryCount:this.queryPlan.plan.length,plannedQueries:this.queryPlan.plan.map(row=>({query:row.query,components:row.components,provenance:row.provenance,specificity:row.specificity})),taskModel:{objective:this.taskModel.objective,targetFormat:this.taskModel.targetFormat,formatPolicy:this.taskModel.formatPolicy,constraints:this.taskModel.constraints,successEvidence:this.taskModel.successEvidence},capabilities:this.capabilityCatalog});
    ...
    try{
      ...
      this.writeBatch(this.status,true);await this.body.finishTask(this.task.taskId,'COMPLETED',{status:this.status,targetVideoId:this.config.target,targetFormat:this.taskModel?.targetFormat||FORMAT.UNKNOWN,targetOpened:this.targetOpened,steps:this.stepNo,planner:'v3'});return this.reportObject(this.status);
