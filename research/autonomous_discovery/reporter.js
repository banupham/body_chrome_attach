...
function reportMarkdown(report){
  ...
  lines.push('');
  if(report.autonomy?.taskModel){const t=report.autonomy.taskModel;lines.push('## Brain task understanding','');lines.push(`- Objective: **${esc(t.objective)}**`);lines.push(`- Target topic: ${esc(t.targetTopic)}`);lines.push(`- Target language: ${esc(t.targetLanguage)}`);lines.push(`- Constraints: ${(t.constraints||[]).map(esc).join(', ')||'none'}`);lines.push(`- Success evidence: ${(t.successEvidence||[]).map(esc).join(', ')||'n/a'}`,'');}
  ...
  if(report.autonomy?.mode==='FULL_BODY_AGENT'){
    lines.push('| Step | Subgoal | Capability | Purpose | Query/Video | Changed? | Change evidence | Reward | Success |');
    lines.push('|---:|---|---|---|---|---|---|---:|---|');
    for(const row of report.path||[])lines.push(`| ${row.step} | ${esc(row.subgoal)} | ${esc(row.capability)} | ${esc(row.purpose)} | ${esc(row.query||row.videoId||'')} | ${row.changed?'yes':'no'} | ${esc((row.changeReasons||[]).join(','))} | ${row.reward??''} | ${row.success?'yes':'no'} |`);
  }else{
  ...
  lines.push('','> The target video ID/title remain success/profile evidence and are not permitted as direct YouTube search queries. Query text is generated from target metadata plus target-anchored terms learned from the observed environment.');
  return lines.join('\n')+'\n';
}
