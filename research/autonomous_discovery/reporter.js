...
function reportMarkdown(report){
  ...
  lines.push('');
  if(report.account){const a=report.account;lines.push('## Account context','');lines.push(`- Account key: \`${esc(a.accountKey||'unknown')}\``);lines.push(`- History sample: ${a.historySampleCount??0} videos`);lines.push(`- Profile confidence: ${a.profileConfidence??0}`);lines.push(`- Target relationship: **${esc(a.relationship?.kind||'UNKNOWN')}**`);lines.push(`- Relationship reason: ${esc(a.relationship?.reason||'n/a')}`);lines.push(`- Target previously watched: ${a.relationship?.targetPreviouslyWatched?'yes':'no'}`);lines.push(`- Account-aware plan: **${esc(a.plan?.mode||'none')}**`);lines.push(`- Dominant topics: ${(a.habits?.dominantTopics||[]).map(x=>`${esc(x.key)}=${x.count}`).join(', ')||'n/a'}`);lines.push(`- Preferred formats: ${(a.habits?.preferredFormats||[]).map(x=>`${esc(x.key)}=${x.count}`).join(', ')||'n/a'}`);lines.push(`- Profiling BODY actions: ${a.profileActions??0}`,'');}
  if(report.autonomy?.taskModel){const t=report.autonomy.taskModel;lines.push('## Brain task understanding','');lines.push(`- Objective: **${esc(t.objective)}**`);lines.push(`- Target topic: ${esc(t.targetTopic)}`);lines.push(`- Target language: ${esc(t.targetLanguage)}`);if(t.accountContext)lines.push(`- Account relationship used by planner: ${esc(t.accountContext.relationship?.kind||'UNKNOWN')}`);lines.push(`- Constraints: ${(t.constraints||[]).map(esc).join(', ')||'none'}`);lines.push(`- Success evidence: ${(t.successEvidence||[]).map(esc).join(', ')||'n/a'}`,'');}
  ...
  lines.push('','> The target video ID/title remain success/profile evidence and are not permitted as direct YouTube search queries. Account watch history is used only to build a local content-affinity context; it is not used as a direct target navigation shortcut.');
  return lines.join('\n')+'\n';
}
