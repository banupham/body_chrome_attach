...
function commandCountOf(runner){return Array.isArray(runner?.commandIds)?runner.commandIds.length:0;}
...
class TargetPlanPortfolioRunner extends RobustKeyGraphRouteRunner{
  ...
  async executePlan(plan){
    ...
    const evaluation=evaluatePlanRun(run,this.targetFingerprint,{elapsedMs:Date.now()-started,commandCount:Math.max(0,commandCountOf(this)-commandStart)});const result={plan,run,evaluation};this.planResults.push(result);this.headKeywordRuns.push(run);if(evaluation.found)this.anyTargetObserved=true;this.log('target_plan_complete',{planId:plan.id,query:plan.query,traversal:plan.traversal,found:evaluation.found,routeType:evaluation.bestRoute?.routeType||null,graphEdgeCount:evaluation.bestRoute?.graphEdgeCount??null,closestScore:evaluation.closestObserved?.proximity?.score||0,elapsedMs:evaluation.elapsedMs,commandCount:evaluation.commandCount});return result;
  }
  async run(){
    try{
      ...
      if(this.config.homeExplore!==false){const oldWait=this.config.homeWaitMs,oldScrolls=this.config.homeScrolls;this.config.homeWaitMs=Math.min(Number(oldWait||8000),2500);this.config.homeScrolls=0;try{this.homeDiscovery.baseline=await this.scanHome('target_portfolio_baseline',{navigate:false});if(!this.homeDiscovery.baseline?.candidateCount)this.log('target_portfolio_baseline_empty',{reason:'fresh_signed_out_home_has_no_candidates',waitMs:this.config.homeWaitMs,continued:true});}finally{this.config.homeWaitMs=oldWait;this.config.homeScrolls=oldScrolls;}}
