...
function loadClientToken(file=DEFAULT_TOKEN_PATH){const env=String(process.env.BODY_DEBUG_TOKEN||process.env.BODY_DAEMON_TOKEN||'').trim();if(env)return env;try{const value=fs.readFileSync(file,'utf8').trim();if(value)return value;}catch{}throw new Error(`Không tìm thấy debug auth token tại ${file}. Hãy chạy BODY.exe hoặc BODY runtime trước.`);}
...
class BodyDebugClient{
  ...
  async connect(){
    ...
    await new Promise((resolve,reject)=>{const ws=this.socket=new WebSocket(url);const timer=setTimeout(()=>reject(new Error(`Không kết nối được Company Runtime ${url}. Hãy chạy BODY.exe hoặc BODY runtime trước.`)),8000);const fail=error=>{clearTimeout(timer);reject(error instanceof Error?error:new Error(String(error)));};
