...
class BodyDebugClient{
  ...
  async connect(){
    ...
    await new Promise((resolve,reject)=>{const ws=this.socket=new WebSocket(this.url);const timer=setTimeout(()=>reject(new Error(`Không kết nối được Body runtime ${this.url}. Hãy chạy daemon.cmd trước.`)),8000);const fail=error=>{clearTimeout(timer);reject(error instanceof Error?error:new Error(String(error)));};
      ...
      ws.on('message',raw=>{let msg;try{msg=JSON.parse(String(raw));}catch{return;}if(msg.type==='HELLO_ACK'&&msg.authenticated===true){clearTimeout(timer);this.authenticated=true;this.controller=msg.controller||null;resolve();return;}if(msg.type==='AUTH_ERROR'){fail(new Error(`Body runtime auth failed: ${msg.error||'unknown'}`));return;}this.onMessageObject(msg);});
      ws.on('close',()=>{this.authenticated=false;const error=new Error('Body runtime disconnected.');for(const item of this.pending.values()){clearTimeout(item.timer);item.reject(error);}this.pending.clear();});
    });
  }
...
async function interactive(client){await client.connect();console.log(`Đã kết nối Body runtime ${client.url} với vai trò DEBUG CLIENT.`);if(client.controller?.brainOnline)console.log('Brain đang giữ quyền điều khiển: các lệnh test có tác dụng sẽ bị chặn; lệnh đọc vẫn dùng được.');const rl=readline.createInterface({input:process.stdin,output:process.stdout,prompt:'BODY-DEBUG> '});rl.prompt();rl.on('line',async line=>{if(!String(line).trim())return rl.prompt();rl.pause();try{print(await client.command(line));}catch(error){console.error('[LỖI]',String(error?.message||error));}finally{rl.resume();rl.prompt();}});await new Promise(resolve=>rl.once('close',resolve));}
