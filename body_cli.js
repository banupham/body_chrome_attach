...
const DEFAULT_ENDPOINT_PATH=path.join(__dirname,'daemon','state','runtime-endpoint.json');
...
class BodyDebugClient{
  ...
  async connect(){
    ...
    await new Promise((resolve,reject)=>{const ws=this.socket=new WebSocket(url);const timer=setTimeout(()=>reject(new Error(`Không kết nối được Company Runtime ${url}. Hãy chạy daemon.cmd trước.`)),8000);const fail=error=>{clearTimeout(timer);reject(error instanceof Error?error:new Error(String(error)));};
...
module.exports={PROTOCOL_VERSION,DEFAULT_TOKEN_PATH,DEFAULT_ENDPOINT_PATH,loadClientToken,loadRuntimeUrl,BodyDebugClient,BodyDaemonClient:BodyDebugClient,main};
